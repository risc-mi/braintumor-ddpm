...
class PixelRepresentationsTrainer(SimpleNetworkTrainer):
    def __init__(self,
                 network: nn.Module,
                 output_folder: str,
                 images_dir: str,
        ...
        super(PixelRepresentationsTrainer, self).__init__()
        ...
        self.use_validation_from_train = True
    ...
    def initialize_data_loaders(self) -> None:
        ...
        # Create training data from pixels
        if self.use_validation_from_train:
            self.valid_data = self.train_data
        pixel_data_train = self.prepare_pixel_data(data=self.train_data)

        # Setup data-loaders
        self.train_dl = DataLoader(dataset=pixel_data_train, batch_size=self.batch_size, shuffle=True)
    ...
    def prepare_pixel_data(self, data: Dataset) -> PixelDataset:
        ...
        # Create empty tensors to hold all image/label data
        x = torch.zeros(size=(data.__len__(), self.network.in_features, *image_size), dtype=torch.float32)
        y = torch.zeros(size=(data.__len__(), *image_size), dtype=torch.uint8)
        ...
        for i in range(0, data.__len__()):
            ...
            features = self.network.feature_extractor(image)
            x[i] = features
            y[i] = mask

        # Adjust tensor shapes
        x = x.permute(1, 0, 2, 3).reshape(self.network.in_features, -1).permute(1, 0)
        y = y.flatten()

        data = PixelDataset(x_data=x, y_data=y, device=self.device)
        return data

    def update_training_data(self) -> None:
        """ Updates training data by applying random augmentation and recreating a new PixelDataset """
        if self.online_augmentation is not None:
            self.write_to_log_file(f"Applying online-augmentation and reconstructing data\n")
            updated_pixel_data = self.prepare_pixel_data(data=self.train_data)
            self.train_dl = DataLoader(dataset=updated_pixel_data, batch_size=self.batch_size, shuffle=True)
        ...
        try:
            if self.is_initialized:
                ...
                self.optimizer = torch.optim.Adam(
                    params=self.network.feature_classifier.parameters(),
                    lr=self.initial_lr,
                    amsgrad=True
                )
    ...
    def on_epoch_end(self) -> bool:
        ...
        if (self.epoch != 0) and (self.epoch % self.augment_every == 0):
    ...
    def predict_test_data(self, save_predictions: str = 'none') -> None:
        """
